...
public class Main extends std {
    ...
    public static void cont(){
        ...
        stdout.println("已連接");
        slp(2000L);
        stdout.println("[D3f4ult] 你好 我是GigaByte的助手 現在請跟者我一步一步操作 操作完你就學會了...三分之一");
        slp(1000L);
    ...
    private static void scan(){
        ...
        stdout.println("1. Jhon's Server (182.124.65.32)");
        stdout.println("2. Hoyiqiang's NAS (192.168.31.4)");
        do {
            stdout.print("[D3f4ult] 接著輸入182.124.65.32 \nTip:connect就是連線 182.124.65.32就是電腦的IP\n[mysvr] ~$ C:/");
        }while (!sc.nextLine().equalsIgnoreCase("connect 1"));
        cnt1();
    }
    ...
    private static void scan2(){
        stdout.println("以下是目前在區網上找的到的電腦or伺服器");
        stdout.println("1. Jhon's Server (182.124.65.32)");
        stdout.println("2. Hoyiqiang's NAS (165.212.78.22)");
        stdout.println("3. huirohbuhrupieahiui (666.666.666.666)\n");
        slp(2000L);
        do{
            stdout.print("[D3f4ult] 接者connect 666.666.666.666\n[mysvr] ~$ C:/");
        }while (!sc.nextLine().equalsIgnoreCase("connect 666.666.666.666"));
        cnt3();
    }
}
